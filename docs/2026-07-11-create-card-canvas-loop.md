...
## F7 · 2026-07-12 ✅ 画布重排(主理人指出 UI 问题:画布大小完全不够)

- 三处病根(截图诊断):①F3 指示行塞进 .create-field 旧 flex 容器成第三列,被挤成两字宽竖条(「想怎」悬空)——F 系列自己引入的回归;②三栏(会话 400+画布+产出 240)把 680 上限的画布纸面挤到 ~640,正文列只剩 ~440px,长字段(性格 300+ 字)读成一行 12 字的窄报纸;③✦/⟳/批准的合成指令全文裸奔在会话流(「请直接把「性格」(personality)改写得更具体……」)。
- 重排:
  - **产出侧 240px 列退场**:本台架/收纳/装订/发布收编成画布顶部**台账线**一行(hairline,少框)——已建名字签(可点开 builtView,draggable=F4 拖拽源保留)+「装订 N 卡」浮层(清单与 ⚠ 提示原样)+ 收进本台/收入卡库/发布三钮 +「⋯」菜单(从卡库补素材/导出 JSON/从故事继续改);移除/导出在 builtView 弹窗本就有,功能零丢失。DepthCard 台架卡退役。
  - **画布放宽**:纸面 680→880 上限,stage 单列;会话栏 clamp(320,26vw,400)→(300,24vw,360)。
  - **字段块式**(桌面):字段名+动作一行(名行自留 26px 高),正文全宽在下,阅读行宽上限 620px;指示行同宽——窄槽 bug 随布局根治。手机基础规则未动。
  - **指令人话化**:sendText 加 display 选项,消息存 {text(完整指令,API 载荷), show(短句,UI 显示)};✦/⟳ 显示「✦ 补写「简述」—— 要求」、批准显示「批准蓝图,开始落笔」;旧消息无 show 键 fallback 原文,localStorage 双向兼容。
- 实测(1366):画布 768px(原~640)、正文列 620px(原~440)、指示行 620px 全宽正常展开;台账线三钮+装订浮层+菜单全通,外点即关;✦ 空回车默认补写成功,会话流只见短句、localStorage text 仍为完整指令(双轨证实);手机 390:.ct 原样、台账线不渲染、人话化短句同步生效;console 全程零报错;build 过。
- 备注:本片是对 E1 双栏骨架的修缮(布局问题在真实长字段内容下才暴露——测试数据都是短字段);preview 截图 30s 超时(环境已知),验收以几何度量+DOM 断言为准。

## F7b · 2026-07-12 ✅ 完善角色卡弹窗左右结构(主理人指定)

- 原纵排(立绘大图→头像→名字→介绍)一柱到底,弹窗 520 窄+长滚动;改**桌面左右结构**:弹窗 780 宽,左栏 240=立绘主图吃满,右栏=头像+名字横排一行 + 角色介绍(介绍框 rows 4→7,右栏高度用起来);手机(≤860)保持原纵排居中,零影响。
- 实测:1366 弹窗 780、两栏 240|450 并排、头像行 row、名字在头像旁;390 回退纵排(column+上下堆叠);console 零错;build 过。

## F8 · 2026-07-12 ✅ 画布即主界面(主理人拍板:助手缩成很小的一部分)

- 布局层级反转(E1 双栏骨架退役):**画布=唯一主界面**(880 全幅,1366 实测拿满),左会话栏整个退场。
- 助手压缩成三件小东西:
  - **dock 命令条**贴画布下沿一条(与画布同宽):输入/上传/挂资料/@ 引用/发送原样,新增「对话 · N」按钮;
  - **批注行**(落笔期):dock 顶一行小字显示助手最新一句(单行截断,点开抽屉);构思期不占 dock(那时助手在画布上说话);
  - **对话手记抽屉**:完整历史按需左滑展开(稿纸对谈体原样迁入,shade 点外关,drawerIn 动画+reduced-motion 兜底)。
- **构思上画布**:聊开(或已有构思产物——messages>1 / questions / blueprint 任一)后,火候线+助手追问(17px 大字)+圈选词+蓝图批准直接长在画布纸面上(create-muse,max-width 640 居中)——构思就是在纸上打草稿,「卡即界面」贯彻到构思期;空台首次仍是印章+起手句+入口行。
- 实测(5199):会话栏 DOM 消失、画布 880、dock 同宽、批注行显最新一句、「对话 · 7」开抽屉(7 条对谈体,shade 关);blueprint 态蓝图/附言/批准在画布上;understand 态火候线 30+追问+3 圈选词+填空线在画布上(修了一个边界:blueprint 有产物但 messages≤1 时被旧条件拦住,改为"聊开或有构思产物");手机 390:.ct 原样、muse/drawer/dock 零泄漏;console 零错;build 过。
- 如实边界:桌面会话不再常驻可见——构思期助手的话在画布上、落笔期在批注行,完整历史差一次点击;若实际用下来"批注行太隐蔽",可以把抽屉改成常开细栏(留口子,等真实使用反馈)。

## G0-G3 · 2026-07-12 ✅ Lovart 式全屏画板(挂版+自由拖放,一体落地)

- 主理人拍板:做成类似 Lovart 的全屏挂版;两确认:**四台合一块板**(项目全景)+**自由拖放**(pan/zoom 留二期)。计划经 plan mode 批准(G 系列)。
- **G0 骨架**:大标题/kind tabs/F7 台账线全部退役成一条 boardbar(左=四个「+ 类型」新建;右=装订浮层/发布/对话·N/⋯菜单);画板吃满剩余视口(素纸+26px 极淡点阵=挂版感);四台 draft+built 全部投影成 224px 纸卡(kind 徽标+楷体名+摘要两行;draft 卡朱砂呼吸点);dock 变底部中央浮岛(fixed,720px,纸面圆角+深影)。desks 零改动——板只是投影。
- **G1 拖放**:pointer 拖(>4px 判拖,否则单击选中);坐标存 `ais_create_board_v1`(视图层,回滚=删一个键);实测拖 (140,90) 位移 → translate(188,114) 精确持久化;无坐标卡按 kind 锚区自动落位(修了一个 bug:偏移误用全局序号,改 kind 内序号)。拖到 dock 浮岛上=挂引用(F4 语义保留)。
- **G2 交互面**:单击选中=朱砂描边+ki 跟随+dock 解锁该台语境(「说说这个角色…」);点空白=取消(dock 禁用+引导语);built 卡 hover 五动作(查看/引用/改编/导出/移除,全部复用现成函数——引用实测落纸签);「+ 演出卡」空台新建=直接聚焦构思(blank 起手句 3 个+dock 演出语境);空板中央引导(模板/导入/挂资料)。
- **G3 聚焦态**:双击=overlay 放大(920px 纸面,4vh 顶距);draft 聚焦=F8 画布 JSX **原样全套**(字段块式/✦ 指示行/构思 muse/收进本台+收入卡库在顶栏);built 聚焦=只读字段(无编辑钮);Esc/回画板/点罩层三路退出。**闭环铁证**:聚焦态 ✦ 默认补写真调(8017)→draft 16 键、fresh 墨晕、**板上卡片摘要实时变**(「深夜电台《零点之后》的主持人…」)——选中即聊、AI 改动直接映射回板。
- 回归:手机 390 `.ct` 原样(board/boardbar/focus 零泄漏);console 全程零错;build 过。
- 如实边界:①built 卡不能直接聊(dock 明示"先改编成草稿"),draft 才是可聊对象;②一期固定视口(板内 overflow 滚动兜底),pan/zoom/框选/连线明确留二期;③依赖 ki 的弹窗语境=最后选中的台。
