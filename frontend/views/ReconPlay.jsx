      ...
      <div className="main">
        ...
        <div className="presence">
          ...
          {dialogues ? dialogues.map((d, i) => (
            <div className="prow" key={i}><span className="avi">{ini(d.name)}</span><span className="nm" style={{ width: (d.name || "").length > 2 ? "72px" : undefined }}>{d.name}</span><span className="dash"></span><span className="ln-q">{d.text}</span></div>
          )) : (
        ...
        <div className="choices">
          ...
          <div className="cgrid">
            {choices ? choices.slice(0, 4).map((c, i) => (
              <div className="ccard" key={i} onClick={() => !busy && onChoice(c)} style={{ cursor: busy ? "default" : "pointer" }}>
