-- 账户系统 Phase 1:用户 + token + 邮箱验证码 + 角色 + 数据归属(会话/卡库/预设按人隔离)
...
-- 1) 用户。email=已验证主身份(注册走邮箱验证码);username 可选;role=user/admin/superadmin。
--    superadmin 由 .env SUPERADMIN_EMAIL 钉(无需写进 role 列)。
...
-- 5) 卡库/预设唯一性改造:旧全局唯一约束 → 官方行(NULL)与用户行各自部分唯一。
--    旧约束被 save_library/save_preset 的 on conflict 引用,storage.py 已改 update-then-insert,故可安全 drop。
--    约束名为 Postgres 内联 unique 的默认名(线上若不同,按 \d 实名调整)。
