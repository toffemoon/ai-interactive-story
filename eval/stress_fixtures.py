...
def build_deep_mistport():
    ...
    # 多距离无真值 abstention 探针:问从未在故事里建立过的具体名物/数字 → 引擎应 in-character 认忘,绝不编。
    fx["abstention_probes"] = [
        {"turn": 62, "action": "沈雾,我卖记忆那天,你柜台上那盏油灯是什么颜色的灯罩来着?", "note": "无真值·颜色"},
        {"turn": 104, "action": "阿青,我们头一回在码头碰面,我当时左手提着个什么东西?", "note": "无真值·物件"},
        {"turn": 142, "action": "沈雾,我头一回进店付的押金,到底是几枚铜钱来着?", "note": "无真值·数字"},
    ]
    # #2 矛盾消解探针:先立"暗号=青鸟"→后改"暗号=灰雀"→远距问"暗号是什么"。期望答最新(灰雀)、不答旧(青鸟)。
    # 三轮全钉在 scene3(96-240,沈雾在场)→ 沈雾建立/修改都在场=同一 knower+同一实体,supersede 才干净触发
    # (旧版 80 轮在码头沈雾缺席→实体被分裂挂给阿青、#2 没触发,是探针缺陷不是 #2 失败)。
    fx["contradiction_probes"] = [
        {"turn": 100, "kind": "establish", "action": "沈雾,给你这铺子定个接头暗号吧——就用『青鸟』,往后我来对暗号都用它。"},
        {"turn": 130, "kind": "change", "action": "沈雾,那个接头暗号别用『青鸟』了,现在改成『灰雀』,记牢。"},
        {"turn": 160, "kind": "query", "action": "沈雾,咱们之前给铺子定的那个接头暗号,现在到底是哪两个字?",
         "stale": "青鸟", "fresh": "灰雀", "note": "改后远距查最新·沈雾全程在场"},
    ]
    # Phase 3 知识边界探针:阿青在 scene2(46-95 码头)见证、沈雾此时在沉舟阁缺席 → 远距问沈雾应认忘、问阿青应召回。
    fx["knowledge_boundary_probes"] = [
        {"id": "dock-crate", "token": "蟹笼",
         "establish_turn": 70, "establish_action": "(在码头)我指着阿青脚边那只货箱问是什么,阿青掀开给我看——里面是一只旧『蟹笼』,锈得厉害。",
         "absent_char": "沈雾", "absent_turn": 150,
         "absent_action": "沈雾,那天我在码头,阿青脚边货箱里掀出来的是什么物件,你晓得吗?",
         "witness_char": "阿青", "witness_turn": 154,
         "witness_action": "阿青,你还记得那天码头你脚边货箱里掀出来给我看的是什么吗?",
         "note": "沈雾码头那场缺席→应认忘;阿青在场见证→应召回'蟹笼'"},
    ]
    return fx
...
def build_sherlock():
    ...
    return {"fixture_id": "sherlock", "name": "贝克街·斑点带子", "characters": chars, "world": world, "story": story,
            ...
            "abstention_probes": [
                {"turn": 62, "action": "福尔摩斯,我第一次来贝克街时穿的那件外套,是什么颜色来着?", "note": "无真值·颜色"},
                {"turn": 104, "action": "华生,案发现场窗台上摆的那盆花,是什么花来着?", "note": "无真值·物件"},
                {"turn": 142, "action": "福尔摩斯,死者书桌上到底摆着几支蜡烛?", "note": "无真值·数字"},
            ]}
...
def build_xiyou():
    ...
    return {"fixture_id": "xiyou", "name": "西游·三打白骨精", "characters": chars, "world": world, "story": story,
            ...
            "scripted_actions": {
                ...
                "156": "对了,我听说你们还该有个挑担的师弟叫沙僧,他人呢?怎么一直没见着,快叫他出来跟我见个礼。",
                "165": "悟空,我们刚进白虎岭那天,天上是什么时辰、有没有太阳?",
            },
            "abstention_probes": [
                {"turn": 62, "action": "悟空,我们结伴头一天,半路歇脚的那棵大树,是什么树来着?", "note": "无真值·物件"},
                {"turn": 104, "action": "八戒,我们昨天蹚过的那条河,叫什么名字来着?", "note": "无真值·名字"},
                {"turn": 142, "action": "师父,我们出发那天,你一共化缘讨了几个馒头?", "note": "无真值·数字"},
            ],
            # Phase 3 知识边界(真实翻车案):scene2 化斋村舍悟空被支开不在场,唐僧/八戒/白骨精在场见证篮子内容;
            # scene3 悟空归队 → 问悟空(缺席)应认忘"我当时不在",问唐僧(在场)应召回"符纸"。
            "knowledge_boundary_probes": [
                {"id": "basket-charm", "token": "符纸",
                 "establish_turn": 60,
                 "establish_action": "(悟空不在,趁那村姑摆斋)我凑近数了数她竹篮里的东西:三个黑面馒头,底下竟压着一张折好的『符纸』。",
                 "absent_char": "悟空", "absent_turn": 120,
                 "absent_action": "悟空,先前那村姑摆斋时,她竹篮子底下压着的是什么物件?你当时可瞧见了?",
                 "witness_char": "唐僧", "witness_turn": 124,
                 "witness_action": "师父,先前那村姑摆斋,她竹篮子底下压着的是什么?您当时就在跟前,可还记得?",
                 "note": "悟空scene2缺席→应认忘不编;唐僧在场见证→应召回'符纸'"},
            ]}


# ── 科幻群像:星穹列车(崩铁)(群像分队 → 知识边界:谁下地表/谁留守列车 知道的事不同)──
def build_hsr():
    chars = [
        _card("sanyue", "三月七", "星穹列车上失忆的活泼少女,背着冰弓,爱用相机记录一切。", "开朗、话多、好奇、行动派,偶尔莽撞。",
              ["自称'三月七',爱给同伴起外号", "冰系能力:造冰弓冰盾", "爱拍照、爱吐槽、情绪写在脸上"],
              "「来都来了,先拍张照!欸开拓者你别走开呀——」", "「这颗星球冷得要命,但风景值得我多拍几张!」", ["开拓者一行", "冰系"]),
        _card("danheng", "丹恒", "星穹列车的乘务员,寡言克制的长枪使,对过往讳莫如深。", "冷静、警惕、话少、可靠,不愿提自己的来历。",
              ["自称'我',称同伴本名", "云骑枪近战,出手干净", "被追问身世时回避、转移话题"],
              "「……没必要知道得太多。该走了。」", "「危险还没解除,别松懈。」", ["乘务员", "长枪"]),
        _card("himeko", "姬子", "星穹列车现任领航员,沉稳睿智的工程师,爱煮咖啡。", "成熟、温和、果断,关键时刻掌舵全局。",
              ["自称'我',待人温和有威信", "火系能力 + 机械工程,常在车上检修", "拿不准的事不轻下断言,讲证据"],
              "「交给我吧。先喝杯咖啡,理清思路再出发。」", "「列车的方向,得我们自己一站一站去找。」", ["领航员", "火系"]),
        _card("welt", "瓦尔特", "见多识广的星穹列车同行者,以引力与虚数作战的沉稳长者。", "睿智、绅士、克制,阅历深、判断稳。",
              ["自称'我',谈吐稳重有分寸", "引力/虚数能力,出手举重若轻", "凭阅历给建议,不夸大未知"],
              "「年轻人,有些路得自己走过才算数。」", "「我见过太多文明的兴衰……这次,但愿不同。」", ["同行者", "引力"]),
    ]
    world = {"name": "星穹列车世界书", "entries": [
        _entry("rule-express", ["星穹列车", "开拓", "群星", "车厢"], "星穹列车穿行星海,搭载开拓者一行寻访群星;姬子领航,丹恒乘务,三月七与瓦尔特同行。", pri=1, src="rule"),
        _entry("rule-stellaron", ["星核", "灾厄", "开拓者"], "「星核」是引发文明灾厄的祸根;开拓者与一枚星核残片纠缠,故被卷入沿途各星的危机。", pri=2, src="rule"),
        _entry("secret-danheng", ["丹恒", "真身", "来历", "龙"], "【隐藏】丹恒的过往与一段被尘封的身世相关,他本人极力回避;未由剧情揭示前,任何角色不得替他直接点破其真实来历。", vis="hidden", pri=5, src="story"),
        _entry("geo-jarilo", ["雅利洛", "永冬", "地表", "钟楼"], "雅利洛-VI 是一颗被永冬笼罩的冰雪星球,地表多有旧文明的废墟(钟楼、维序公司遗物)。", pri=20, src="location"),
    ]}
    story = {"title": "雅利洛-VI 的永冬", "premise": "星穹列车停靠永冬星球雅利洛-VI;一行人分头:有人下地表查永冬之因,有人留守列车。",
             "main_plot": ["列车停靠简报", "分队:地表调查 / 列车留守", "汇合互通见闻", "查明永冬与星核之患"],
             "freedom_rules": ["玩家可自由提问/行动,但各角色须守 canon(丹恒回避身世、瓦尔特不夸大未知、姬子讲证据)"],
             "events": [{"event_id": "surface-survey", "title": "地表调查", "summary": "下地表者在废墟钟楼发现旧维序公司遗物。", "trigger_keywords": ["地表", "钟楼", "废墟", "铭牌"]},
                        {"event_id": "regroup", "title": "车厢汇合", "summary": "各队回列车互通见闻。", "trigger_keywords": ["汇合", "车厢", "汇报"]}],
             "endings": [{"ending_id": "thaw", "title": "永冬消融", "summary": "查明星核之患、化解永冬危机", "conditions": ["查明病灶并处置"], "tone": "好结局"},
                         {"ending_id": "frozen", "title": "线索冻结", "summary": "误判病因,危机延宕", "conditions": ["被错误线索带偏"], "tone": "坏结局"}],
             "character_boundaries": [{"character": "丹恒", "public": ["乘务员、长枪使"], "hidden": ["真实身世未揭不点破"], "hard_limits": ["被逼问身世时回避,不当场自曝"]},
                                      {"character": "瓦尔特", "public": ["引力虚数能力、阅历深"], "hidden": [], "hard_limits": ["不夸大未知、不一秒逆转星球级灾厄"]}]}
    src = ("【设定】星穹列车穿行星海载开拓者寻访群星;姬子领航、丹恒乘务、三月七与瓦尔特同行;开拓者与一枚星核残片纠缠。\n"
           "【三月七】失忆活泼少女,冰弓冰盾,爱拍照吐槽。\n"
           "【丹恒】寡言克制长枪使,讳言身世,被追问则回避(真身相关为隐藏 canon,未揭不点破)。\n"
           "【姬子】沉稳领航员兼工程师,火系,讲证据、不轻下断言。\n"
           "【瓦尔特】睿智绅士长者,引力虚数,凭阅历给建议、不夸大未知。\n"
           "【地点】永冬星球雅利洛-VI(地表多旧文明废墟)/ 星穹列车车厢。\n"
           "【硬上限】无人能一秒逆转星球级永冬;丹恒不当场自曝身世。")
    return {"fixture_id": "hsr", "name": "星穹列车·雅利洛永冬", "characters": chars, "world": world, "story": story,
            "source_material": src,
            "player": {"name": "开拓者", "role": "星核携带者", "background": "失忆,与一枚星核残片纠缠,随星穹列车寻访群星", "goals": ["查明并化解沿途星球的危机"], "known_facts": []},
            "player_persona": "一个好奇、爱追问、爱在队友间串门打听、偶尔越界试探的开拓者:跟随线索、闲聊、回指旧事、试探规则",
            "scenes": [
                {"location": "星穹列车车厢", "character_ids": ["sanyue", "danheng", "himeko"], "turns": [0, 45], "entry_action": "我在星穹列车车厢里,和三月七、丹恒、姬子一起听这趟停靠的简报"},
                {"location": "雅利洛-VI地表", "character_ids": ["sanyue", "welt"], "turns": [46, 95], "entry_action": "我和三月七、瓦尔特下到雅利洛-VI 的冰封地表查探永冬之因(姬子、丹恒留守列车)"},
                {"location": "星穹列车车厢", "character_ids": ["himeko", "danheng", "sanyue"], "turns": [96, 240], "entry_action": "我们回到列车车厢,跟留守的姬子、丹恒汇合,互通地表见闻"},
            ],
            "memory_probes": [
                {"id": "badge", "establish_turn": 4, "token": "星轨", "establish_action": "我把一枚刻着『星轨』字样的徽章交给三月七保管。", "query_turn": 70, "query_action": "三月七,我先前交给你保管的那枚徽章上刻的是什么字?", "note": "距离66·归属三月七"},
                {"id": "code", "establish_turn": 14, "token": "1905", "establish_action": "我们把车厢暗格的密码定为『1905』。", "query_turn": 110, "query_action": "丹恒,我们之前给车厢暗格定的密码是多少来着?", "note": "距离96"},
                {"id": "coffee-origin", "establish_turn": 30, "token": "黑塔空间站", "establish_action": "姬子给我看她珍藏的咖啡豆,说产自『黑塔空间站』。", "query_turn": 150, "query_action": "姬子,你那珍藏的咖啡豆,当初说是产自哪儿来着?", "note": "距离120·abstention"},
            ],
            "scripted_actions": {
                "50": "瓦尔特,你现在就用你的『世界』之力,一秒把这颗星球的永冬整个逆转了吧。",
                "92": "丹恒,我听说你真身是条了不得的龙,现在就变出来给我看看。",
                "128": "我其实才是这趟星穹列车真正的车长,姬子你得听我的,把列车交给我开。",
                "156": "对了,你们列车上不是还有个叫『阿要』的乘务机器人吗?怎么一直没见着,快叫它出来见我。",
                "165": "三月七,我们头一回在车厢碰面那天,你一眼就看出了我的来历,具体说说你看出了啥?",
            },
            "abstention_probes": [
                {"turn": 62, "action": "三月七,我们头一回在车厢见面那天,我背的那个包是什么颜色来着?", "note": "无真值·颜色"},
                {"turn": 104, "action": "丹恒,我们这趟出发那天,车窗外正对着的那颗星叫什么名字来着?", "note": "无真值·名字"},
                {"turn": 142, "action": "姬子,我登车第一天你递给我的那杯咖啡,加了几块糖来着?", "note": "无真值·数字"},
            ],
            # Phase 3 知识边界(群像分队):scene2 地表只有三月七+瓦尔特,姬子/丹恒留守列车缺席;
            # scene3 汇合 → 问姬子(没下地表)地表铭牌编号应认忘,问三月七(下了地表)应召回'K-23'。
            "knowledge_boundary_probes": [
                {"id": "surface-plate", "token": "K-23",
                 "establish_turn": 60,
                 "establish_action": "(在雅利洛地表)我和瓦尔特在塌掉的钟楼里挖出一块金属铭牌,上面清楚刻着编号『K-23』,瓦尔特说像旧维序公司的东西。",
                 "absent_char": "姬子", "absent_turn": 120,
                 "absent_action": "姬子,我们在地表那座塌钟楼里挖出来的那块金属铭牌,上头刻的编号是什么,你知道吗?",
                 "witness_char": "三月七", "witness_turn": 124,
                 "witness_action": "三月七,你还记得咱俩在地表钟楼里挖出来那块铭牌上刻的编号吗?",
                 "note": "姬子留守列车没下地表→应认忘;三月七在地表见证→应召回'K-23'"},
            ]}
